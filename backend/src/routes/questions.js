...
router.post('/', auth, requireRole('admin'), (req, res) => {
  ...
  const question = addQuestion({ text, options, correctIndex: idx });
  res.status(201).json({ message: 'Question created', question });
});

// GET /api/questions — Candidate: all questions (no correctIndex)
router.get('/', auth, requireRole('candidate'), (req, res) => {
...
router.put('/:id', auth, requireRole('admin'), (req, res) => {
  const question = updateQuestion(req.params.id, req.body);
