...
This folder contains the **React** UI for the Assessment Platform. The backend lives in the repo’s **backend/** folder.

- Run from repo root: `cd frontend && npm install && npm start`
- In development, requests to `/api` are proxied to the backend (default `http://localhost:5001`).
